...
subst_nat : subst_of subst_of_nat -> nat -> nat,
...
Definition congr_vnat {s0 t0: nat} (E0: s0 = t0) : vnat s0 = vnat t0 :=
  ap vnat E0.
...
with ren_inst_vl_ty (xi : ren) (sigma : index -> vl)  (E : (xi >>> var_vl) == sigma) (s : ty):
  ren_ty xi s = s.[sigma]
with ren_inst_vl_vls (xi : ren) (sigma : index -> vl)  (E : (xi >>> var_vl) == sigma) (s : vls):
  ren_vls xi s = s.[sigma]
...
Definition subst_of_nat  : list Type :=
  [].
Definition subst_nat (sigma: subst_of subst_of_nat) (s: nat) : nat := s.

Class AsimplInst_nat (s: nat) (sigma: subst_of subst_of_nat) (t: nat) := asimplInstEqn_nat : (subst_nat sigma) s = t .
Hint Mode AsimplInst_nat + + - : typeclass_instance.

Instance AsimplInstRefl_nat (s: nat) (sigma: subst_of subst_of_nat) : AsimplInst_nat s sigma s |100.
Proof. reflexivity. Qed.

(* Proof written by hand. *)
Instance asimplInst_vnat (s0 s0': _)
(sigma: subst_of subst_of_vl)
(theta_0: subst_of subst_of_nat)
(* (E_0': AsimplSubst_nat sigma theta_0) *)
(E_0: AsimplInst_nat s0 theta_0 s0')
  : AsimplInst_vl (vnat s0) sigma (vnat s0').
Proof.
...
Proof.
  hnf in *; simpl; f_equal.
  - rewrite <- E_0. eauto using subst_eq_vl.
  - rewrite <- E_1. eauto using subst_eq_vls.
Qed.
...
Proof.
  hnf in *.
  rewrite <- E3.
  apply compTrans_subst_subst_vls. intro.
  rewrite E1.
  apply E2.
Qed.
...
Proof.
  hnf in *. intro.
  rewrite <- E'.
  unfold comp_vls. erewrite compTrans_subst_subst_vl; eauto.
Qed.
...
Proof.
  intros s. simpl.
  erewrite AsimplInstInst_vls with (sigma' := sigma_vl) (sigma_tau := theta_vl); try split; auto.
  intro. rewrite <- E_vl'. simpl. apply subst_eq_vl. assumption.
Qed. 
...
Proof.
  ...
  rewrite <- E_vl.
  destruct x; simpl; trivial.
  unfold compren_vls; erewrite ren_inst_vl_vl; reflexivity.
Qed.
...
Proof.
  hnf in *; simpl. f_equal.
-   rewrite <- E_0; eauto using subst_eq_vls.
