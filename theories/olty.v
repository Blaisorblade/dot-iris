...
(* Global Instance rename_olty2 : Rename (olty Σ) :=
  ...
  Global Instance env_oltyped_fin_persistent (Γ : sCtx) vs: Persistent (env_oltyped_fin Γ vs).
  Proof. elim: Γ vs => [|τ Γ IHΓ] [|v vs] /=; apply _. Qed.

  Lemma env_oltyped_transl Γ vs :
    env_oltyped_fin Γ vs -∗ env_oltyped Γ (to_subst vs).
  Proof.
  ...
  Lemma env_oltyped_fin_cl_ρ Γ ρ:
    env_oltyped_fin Γ ρ -∗ ⌜ nclosed_sub (length Γ) 0 (to_subst ρ) ⌝.
