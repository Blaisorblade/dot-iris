...
Section logrel.
  ...
  Ltac unfold_interp := rewrite ?interp_TLater ?interp_TAll ?interp_TNat ?interp_TTMem ?interp_TSel /=.
...
Ltac setoid_unfold_interp :=
  ...
  try setoid_rewrite interp_TTMem; cbn.
...
Section logrel_lemmas.
  ...
  Proof.
    iInduction T as [] "IHT" forall (ρ w); iIntros "#HT //="; unfold_interp; try by (iDestruct "HT" as "[% _]").
