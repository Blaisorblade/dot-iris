...
Section Sec.
  ...
  Definition is_stamped_dty (T: ty) (σ: vls) γ: iProp Σ :=
    (∃ φ, γ ⤇ φ ∧
             ∀ ρ v,
               ⌜ length ρ = length σ ⌝ →
               (* We should demand that ρ is closed, and we should check that FV (T) ⊂ dom σ *)
               dot_interp T ρ v ≡ φ (subst_sigma σ ρ) v)%I.
  Instance is_stamped_dty_persistent T σ γ: Persistent (is_stamped_dty T σ γ).
  Proof. apply _. Qed.
  ...
  (** is_stamped_* are predicates to distinguish syntactic entities/source syntax.
      They also check for free variables at the same time, but that's probably a bad idea.
   *)
  Fixpoint is_stamped_tm (t: tm): iProp Σ :=
    match t with
    | tv v =>
      is_stamped_vl v
    | tapp t1 t2 =>
      is_stamped_tm t1 ∧ is_stamped_tm t2
    | tproj t l =>
      is_stamped_tm t
    | tskip t =>
      is_stamped_tm t
  ...
  with
  is_stamped_vl (v1: vl): iProp Σ :=
    match v1 with
    | var_vl i => True
    | vabs t => is_stamped_tm t
    | vobj ds =>
      let fix is_stamped_dms (ds: dms): iProp Σ :=
          match ds with
          | [] => True
          | d :: ds =>
            is_stamped_dm d ∧ is_stamped_dms ds
          end
        in
          is_stamped_dms ds
    | vnat _ => True
  ...
  with
  is_stamped_dm (d: dm): iProp Σ :=
    match d with
    (* Only nontrivial case *)
    | dty T σ (Some γ) =>
      let fix is_stamped_vls (vs: vls): iProp Σ :=
          match vs with
          | [] => True
          | v :: vs =>
            is_stamped_vl v ∧ is_stamped_vls vs
          end
      in
      is_stamped_ty T ∧
      is_stamped_vls σ ∧
      is_stamped_dty T σ γ
    | dty T σ None => False
    | dvl v1 => is_stamped_vl v1
    end%I
  with
  is_stamped_path (p1: path): iProp Σ :=
    match p1 with
    | pv v => is_stamped_vl v
    | pself p l => is_stamped_path p
    end%I
  with
  is_stamped_ty (T1: ty): iProp Σ :=
    match T1 with
    | TTop => True
    | TBot => True
    | TAnd T1 T2 =>
      is_stamped_ty T1 ∧ is_stamped_ty T2
    | TOr T1 T2 =>
      is_stamped_ty T1 ∧ is_stamped_ty T2
    | TLater T1 =>
      is_stamped_ty T1
    ...
    | TMu T =>
      is_stamped_ty T
    | TVMem l T => is_stamped_ty T
    | TTMem l T1 T2 => is_stamped_ty T1 ∧ is_stamped_ty T2
    | TSel p l => is_stamped_path p
    | TSelA p l T1 T2 => is_stamped_path p ∧ is_stamped_ty T1 ∧ is_stamped_ty T2
    | TNat => True
    end%I.
  ...
  Lemma is_stamped_ty_persistent t: Persistent (is_stamped_ty t)
  with  is_stamped_path_persistent t: Persistent (is_stamped_path t)
  with  is_stamped_vl_persistent t: Persistent (is_stamped_vl t)
  with  is_stamped_tm_persistent t: Persistent (is_stamped_tm t)
  with  is_stamped_dm_persistent t: Persistent (is_stamped_dm t).
  Proof.
    all:
      induction t; simpl;
      try case_match;
      (* For the dty case, get to is_stamped_vls before induction or the IH will
         have the wrong shpe *)
      repeat apply bi.and_persistent;
      try induction l; try apply _.
  Qed.
  Global Existing Instance is_stamped_ty_persistent.
  Global Existing Instance is_stamped_path_persistent.
  Global Existing Instance is_stamped_vl_persistent.
  Global Existing Instance is_stamped_tm_persistent.
  Global Existing Instance is_stamped_dm_persistent.
  ...
  Lemma is_stamped_var i: is_stamped_vl (var_vl i).
  Proof. by []. Qed.

  Definition is_stamped_vls :=
    (fix is_stamped_vls (vs: vls): iProp Σ :=
       match vs with
       | [] => True
       | v :: vs =>
         is_stamped_vl v ∧ is_stamped_vls vs
  ...
  Lemma is_stamped_vls_idsσr n r:
    is_stamped_vls ((idsσ n).|[ren r]).
  Proof.
    iInduction n as [ | ] "IHn" forall (r); first trivial.
    asimpl; iSplit; trivial.
  Qed.

  Lemma is_stamped_vls_idsσ n: is_stamped_vls (idsσ n).
  Proof. pose proof (is_stamped_vls_idsσr n (+0)) as Hr. asimpl in Hr. exact Hr. Qed.

  (** Existence of translations for type member definitions. *)
  Lemma ex_t_dty T n:
    fv_n T n →
    is_stamped_ty T -∗
    (|==> ∃ σ γ, is_stamped_dm (dty T σ (Some γ)))%I.
  Proof.
    intros Hfv. iMod (alloc_sp T) as (γ) "#Hγ".
    iIntros "#HT !> /=".
    iExists (idsσ n), γ; repeat iSplit; try done.
    - iApply is_stamped_vls_idsσ.
    - rewrite /is_stamped_dty.
      iExists (dot_interp T); repeat iSplit; try done.
  ...
  with same_skel_ty (T1 T2: ty): Prop :=
    match (T1, T2) with
    | (TTop, TTop) => True
    | (TBot, TBot) => True
    | (TAnd T11 T12, TAnd T21 T22) =>
  ...
  (* Next, prove existence of stamped terms that share skeletons and type *)
  (* members with original terms. We need not check if the original terms work. *)
  ...
  Lemma same_skel_vl_subst u u' v v':
    same_skel_vl u u' → same_skel_vl v v' →
    same_skel_vl (u.[v/]) (u'.[v'/]).
  Proof.
    move: u'; induction u; destruct u';
    move => Hske Hskv;
