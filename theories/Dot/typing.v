...
Reserved Notation "Γ ⊢[  i  ] T1  <: T2" (at level 74, T1, T2, i at next level).
...
| Subs_typed e T1 T2 i :
    Γ ⊢[ 0 ] T1 <: iterate TLater i T2 → Γ ⊢ₜ e : T1 →
    (*───────────────────────────────*)
...
| pself_typed p T i l:
    Γ ⊢ₚ p : T, i →
    Γ ⊢ₚ pself p l : T, i
| p_subs_typed p T1 T2 i :
    Γ ⊢[ i ] T1 <: T2 → Γ ⊢ₚ p : T1, i →
    (*───────────────────────────────*)
    Γ ⊢ₚ p : T2, i
...
| TAllConCov_stp T1 T2 U1 U2 i:
    ...
    (* Γ ⊢[i] T2, S i <: T1, S i → *)
    (* TLater T2 :: Γ ⊢[i] U1, S i <: U2, S i → *)
    (* Non-tight premises. *)
    Γ ⊢[S i] T2 <: T1 →
    T2.|[ren (+1)] :: Γ ⊢[S i] U1 <: U2 →
    is_stamped_ty (length Γ) getStampTable T2 →
...
| TAllDistr_stp T U1 U2 i:
    ...
    Γ ⊢[i] TAnd (TAll T U1) (TAll T U2) <: TAll T (TAnd U1 U2)
| TVMemDistr_stp l T1 T2 i:
    is_stamped_ty (length Γ) getStampTable T1 →
    is_stamped_ty (length Γ) getStampTable T2 →
    Γ ⊢[i] TAnd (TVMem l T1) (TVMem l T2) <: TVMem l (TAnd T1 T2)
| TTMemDistr_strp l L U1 U2 i:
    ...
    Γ ⊢[i] TAnd (TTMem l L U1) (TTMem l L U2) <: TTMem l L (TAnd U1 U2)
where "Γ ⊢[  i  ] T1 <: T2" := (subtype Γ i T1 T2).
  ...
  Lemma stamped_mut_types Γ :
    ...
    (∀ i T1 T2, Γ ⊢[i] T1 <: T2 → ∀ (Hctx: stamped_ctx getStampTable Γ),
  ...
  Proof.
    eapply typing_mut_ind with
        ...
        (P2 := λ Γ p T i _, ∀ (Hctx: stamped_ctx getStampTable Γ),
          is_stamped_ty (length Γ) getStampTable T)
        (P3 := λ Γ i T1 T2 _, ∀ (Hctx: stamped_ctx getStampTable Γ),
    ...
    - specialize (H Hctx) as [H1 H2]. clear s. induction i => //=. apply IHi. rewrite iterate_S in H2. by inverse H2.
...
Notation "Γ ⊢[  i  ] T1 <: T2" := (subtype Γ i T1 T2).
