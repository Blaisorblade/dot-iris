...
Reserved Notation "Γ |L V |⊢{ l := d  } : T" (at level 74, l, d, T, V at next level).
Reserved Notation "Γ |L V |⊢ ds : T" (at level 74, ds, T, V at next level).
...
Section syntyping_lemmas.
  ...
  Lemma typing_obj_ident_to_typing_mut Γ:
    (∀ e T, Γ |⊢ₜ e : T → Γ ⊢ₜ e : T) ∧
    (∀ V ds T, Γ |L V |⊢ ds : T → Γ |L V ⊢ ds : T) ∧
    (∀ V l d T, Γ |L V |⊢{ l := d } : T → Γ |L V ⊢{ l := d } : T) ∧
    (∀ p T i, Γ |⊢ₚ p : T, i → Γ ⊢ₚ p : T, i) ∧
    (∀ T1 i1 T2 i2, Γ |⊢ₜ T1, i1 <: T2, i2 → Γ ⊢ₜ T1, i1 <: T2, i2).
  Proof.
    eapply stamped_objIdent_typing_mut_ind with
        (P := λ Γ e T _, Γ ⊢ₜ e : T)
        (P0 := λ Γ V ds T _, Γ |L V ⊢ ds : T)
        (P1 := λ Γ V l d T _, Γ |L V ⊢{ l := d } : T)
