...
Section DStpLemmas.
  ...
  Lemma sAnd1_Stp Γ T1 T2 i :
    ⊢ Γ s⊨ oAnd T1 T2 <:[i] T1.
  Proof. rewrite sstpd_eq. pupd; by iIntros "!> /= %ρ %v _ !> [$ _]". Qed.
  ...
  Lemma sStp_And Γ T U1 U2 i :
    ...
    Γ s⊨ T <:[i] oAnd U1 U2.
  Proof.
    rewrite !sstpd_eq; pupd; iIntros "#H1 #H2 !> %ρ %v #Hg".
  ...
  Proof. rewrite sstpd_eq. pupd; by iIntros "!> /= %ρ %v _ !>"; eauto. Qed.
  ...
  Lemma sOr_Stp Γ T1 T2 U i :
    ...
    Γ s⊨ oOr T1 T2 <:[i] U.
  Proof.
    rewrite !sstpd_eq; pupd; iIntros "#H1 #H2 !> * #Hg".
    iSpecialize ("H1" $! ρ v with "Hg"); iSpecialize ("H2" $! ρ v with "Hg").
  ...
  Proof.
    rewrite sstpd_eq.
    by pupd; iIntros "!> %ρ %v #Hg !> [[HS|HT] Hu] /="; [iLeft|iRight]; iFrame.
  Qed.
  ...
  Lemma sStp_Add_LaterN {Γ T i j} :
    ⊢ Γ s⊨ T <:[i] oLaterN j T.
  Proof. rewrite sstpd_eq; pupd; iIntros "!> ** !> $".  Qed.
  ...
  *)
  Lemma sMu_Stp_Mu {Γ T1 T2 i} `{!SwapPropI Σ} :
    ...
    Γ s⊨ oMu T1 <:[i] oMu T2.
  Proof.
    rewrite !sstpd_eq; pupd; iIntros "#Hstp !> %ρ %v Hg".
  ...
  Proof.
    pupd; iIntros "#Hsub !> %ρ Hg"; iSpecialize ("Hsub" with "Hg"); iNext i.
  ...
  Proof.
    rewrite !sstpd_delay_oLaterN_plus; pupd; iIntros "#HsubT #HsubU !> %ρ #Hg %v".
  ...
  *)
  ...
  Proof.
    pupd; iIntros "!> %ρ _ !> %v [#H1 H2]"; rewrite !oVMem_eq.
  ...
  Proof.
    pupd; iIntros "!> %ρ _ !> %v #[H1 H2]"; rewrite !oTMem_eq /dot_intv_type_pred.
  ...
  Proof.
    pupd; iIntros "!> %ρ _ !> %v". rewrite !oVMem_eq.
  ...
  *)
  ...
  Lemma sP_Later {Γ} p T i :
    ...
    Γ s⊨p p : T, i.+1.
  Proof.
    pupd; iIntros "#HpT !> %ρ #Hg"; rewrite -later_laterN laterN_later.
