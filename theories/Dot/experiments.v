...
Section Russell.
  ...
  Definition russell_p : listVlC -n> vlC -n> iProp Σ := λne ρ v,
    (⟦ TTMem "A" TBot TTop ⟧ [] v ∧ □ (⟦TSel (pv v) "A"⟧ [] v → False))%I.
  Context (s: stamp).
  ...
  Lemma russellImpl: s ↝ (λ ρ v, russell_p ρ v) -∗
    ▷russell_p [] v -∗ (russell_p [] v -∗ False).
  Proof.
    iIntros "#Hs #HrussellVL #HrussellV".
    iApply "HrussellV". iClear "HrussellV".
    - iSplitL => //.
      iExists (russell_p []), (dtysem [] s).
      repeat iSplit; last (iNext; iApply "HrussellVL").
      all: repeat (repeat iSplit => //; repeat iExists _).
  Qed.
  (* Lemma revImpl: s ↝ (λ ρ v, russell_p ρ v) -∗
    □ (▷ russell_p [] v -∗ False) -∗ russell_p [] v.
  Proof.
    iIntros "#Hs #HnotlRussellV".
    iPoseProof (vHasA with "Hs") as "HvHasA".
    - iSplitL; first by iApply vHasA.
      iIntros "!> #HrussellV0".
      iPoseProof "HrussellV0" as (Hcl) "HrussellV".
      iApply "HnotlRussellV".
      iEval (cbn) in "HrussellV".
      iSplit; first by iApply vHasA.
      iIntros "!>!> #H".
      iApply "HnotlRussellV".
      iSplit; first by iApply vHasA.
  ...
  (* Lemma russellEquiv: s ↝ (λ ρ v, russell_p ρ v) -∗
    ▷ russell_p [] v ∗-∗ (russell_p [] v -∗ False).
  Proof. iIntros; iSplit. by iApply russellImpl. *)

  Lemma notRussellV: s ↝ (λ ρ v, russell_p ρ v) -∗ russell_p [] v → False.
  Proof. iIntros; by iApply russellImpl. Qed.


  Lemma notRussellV_old: s ↝ (λ ρ v, russell_p ρ v) -∗ russell_p [] v → False.
  Proof.
    iIntros "#Hs #HrussellV0".
    iPoseProof "HrussellV0" as "#[HvHasA #HnotRussellV]".
    (* Either: *)
    (* iApply "HnotRussellV". *)
    ...
    - iSplitL => //.
      iExists (russell_p []), (dtysem [] s).
      repeat iSplit; last iApply "HrussellV0";
...
Section Russell.
  ...
  Lemma notUAU: s ↝ (λ ρ v, russell_p ρ v) -∗
    uAu v -∗ ▷ False.
  Proof.
  ...
  Lemma revImpl: s ↝ (λ ρ v, russell_p ρ v) -∗
    □ ▷ (uAu v -∗ False) -∗ uAu v.
  Proof.
    iIntros "#Hs #notVAV". rewrite /uAu.
    iSplit => //.
    iExists (russell_p []), (dtysem [] s).
    repeat (repeat iSplit => //; repeat iExists _).
    iIntros "!>!>!> #H". by iApply "notVAV".
  Qed.

  Lemma impl: s ↝ (λ ρ v, russell_p ρ v) -∗
    uAu v -∗ □ ▷ (uAu v -∗ False).
  Proof.
    iIntros "#Hs #Huau !>".
    iPoseProof (notUAU with "Hs Huau") as "#HF".
    by iNext.
  Qed.


  Lemma uauEquiv: s ↝ (λ ρ v, russell_p ρ v) -∗
    □ ▷ (uAu v -∗ False) ∗-∗ uAu v.
  Proof.
    iIntros "#Hs"; iSplit.
    ...
    - iIntros "#Hvav !>".
      iPoseProof (notUAU with "Hs Hvav") as "#HF".
      by iNext.
  Qed.
