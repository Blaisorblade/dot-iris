...
Section iapprox_instances.
  ...
  Proof. elim => [|???] [|??]; cbn; apply _. Defined.
    ...
    Typeclasses Transparent label.
  Global Instance label_pair_iapprox `{!IApprox Σ A}: IApprox Σ (label * A) :=
    λI '(l1, d1) '(l2, d2), ⌜l1 = l2⌝ ∧ d1 ≈ d2.
  Global Instance label_pair_iapprox_persistent `{!IApprox Σ A} `{!IApproxPersistent A} (x y : label * A):
    Persistent (x ≈ y).
...
Section iapprox_instances.
  ...
  (* Hint Resolve vl_approx dm_approx path_approx : typeclass_instances.
      Hint Transparent IApprox : typeclass_instances. *)
  ...
  Global Arguments iapprox_path /. *)
    ...
    (* Hint Extern 1 (@Persistent (uPredI (iResUR ?Σ)) (@iapprox ?Σ _ (label * ?A) (@label_pair_iapprox ?A ?B) ?x ?y)) => simple apply @label_pair_iapprox_persistent : typeclass_instances. *)
  ...
  Proof.
    all: [> destruct d1, d2| destruct v1, v2| destruct p1, p2]; cbn;
      try apply _.
      apply @list_iapprox_persistent.
      (* Set Debug Tactic Unification. *)
      (* Set Typeclasses Debug Verbosity 2. *)

      Fail apply _.
      (* To fix the issue, use either: *)
      change dm_approx with iapprox_dm.
      (* Or: *)
      (* Hint Transparent IApprox : typeclass_instances. *)
      (* Or the above hint extern. *)
      apply _.
  Qed.
...
Section oproper_instances.
  ...
  Proof.
    ...
    iDestruct 1 as (w1 Heq1 φ1 d1 Hl1) "#[Hl1 Hv1]".
