...
Lemma except_0_wp s E e Φ : (◇ WP e @ s; E {{ Φ }}) ⊢ WP e @ s; E {{ Φ }}.
Proof.
  rewrite wp_unfold /wp_pre. iIntros "H". destruct (to_val e) as [v|] eqn:?.
  { by iMod "H". }
  iMod "H"; auto.
Qed.
Lemma wp_except_0 s E e Φ : WP e @ s; E {{ v, ◇ Φ v }} ⊢ WP e @ s; E {{ Φ }}.
...
Proof.
  ...
  { rewrite except_0_wp. by apply of_to_val in He as <-. }
...
Proof.
  ...
  iIntros (v) "?". iModIntro. by iApply HΦ.
Qed.
...
Section proofmode_classes.
  ...
  Proof. by rewrite /IsExcept0 except_0_wp. Qed.
  ...
  Proof.
    ...
    iApply wp_except_0. iApply (wp_wand with "[Hinner Hα]").
    { by iApply wp_mask_mono; iApply ("Hinner" with "[Hα]"). }
    iIntros (v) "[Hβ HΦ]". iMod "Hβ"; iMod "HΦ".
    iMod ("Hclose" with "Hβ") as "Hclose".
  ...
  Proof.
    ...
    iIntros (v) ">[Hβ HΦ]". iMod ("Hclose" with "Hβ") as "Hclose".
    iModIntro. iApply "HΦ". by iApply "Hclose".
  Qed.
